...
export const SystemHealthCenter: React.FC<HealthCenterProps> = ({
  submissions,
  analyses,
  lang
}) => {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-6 shadow-xl relative overflow-hidden">
      ...
      <div className="absolute right-0 top-0 w-24 h-24 bg-blue-500/5 rounded-full blur-3xl pointer-events-none"></div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        ...
        <div className="flex bg-slate-950 p-1 border border-slate-800 rounded-lg self-start sm:self-auto select-none">
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-center">
        ...
        <div className="md:col-span-4 flex flex-col items-center justify-center p-4 bg-slate-950/40 rounded-xl border border-slate-850 text-center space-y-2">
          ...
          <div className="relative flex items-center justify-center h-28 w-28 mt-1">
            <svg className="w-full h-full transform -rotate-90">
              ...
              <circle
                ...
                strokeDashoffset={2 * Math.PI * 46 * (1 - classIntegrityPercent / 100)}
                strokeLinecap="round"
              />
            </svg>
            <div className="absolute text-center">
              <span className="text-2xl font-black text-white">{classIntegrityPercent}%</span>
              <span className="text-[9px] text-slate-450 block font-bold leading-none">{isAr ? 'معيار النقاء' : 'Pass Rate'}</span>
            </div>
          </div>
        ...
        <div className="md:col-span-8 bg-slate-950/20 p-4 rounded-xl border border-slate-850/80 space-y-3">
          <h4 className="text-[10.5px] font-bold text-slate-300">
      ...
      <div className="bg-slate-950/60 rounded-xl p-4 border border-slate-800/80 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2 w-full sm:w-auto shrink-0 justify-end">
          <button
            ...
            className="w-full sm:w-auto bg-[#312e81]/40 hover:bg-[#4338ca]/50 text-indigo-200 border border-indigo-700/50 hover:border-indigo-650 text-[10.5px] font-bold px-3.5 py-2 rounded-lg transition cursor-pointer flex items-center justify-center gap-1.5 shadow"
          >
      ...
      {successDownloadToast && (
        <div className="absolute bottom-5 inset-x-5 bg-emerald-950/90 border border-emerald-500/40 text-emerald-100 rounded-xl p-3 flex items-center gap-2.5 shadow-xl animate-bounce">
